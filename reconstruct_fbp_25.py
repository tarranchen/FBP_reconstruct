# -*- coding: utf-8 -*-
"""
3D Tomography Reconstruction and Post-Processing Tool v25 (Tkinter GUI)
(English Version: Auto-Start + Advanced Parameters + Clean UI + Auto-Close Popup)

================================================================================
Change Log (v25):
1. [Version] Updated to v25.
2. [GUI] Default Window Size increased to 600x800.
3. [GUI] Optimized Completion Behavior:
   - Success popup now auto-closes after 1 second.
   - Emits a system bell sound upon completion.
   - No need to click "OK" manually.
4. [GUI] Renamed Advanced Parameters labels (from v24 Fixed):
   - "Ring Cutoff Order", "Padding Divisor Order".
================================================================================
"""
...
DEFAULT_CONFIG = {
    "FILE_PATTERN": '*.tif',
    "BINNING_FACTOR": 1,
    "TOLERANCE_VALUE": 25,  # Default: 25
    "MEDIAN_FILTER_RADIUS": 1,  # Default: 1
    "GAUSSIAN_SIGMA": 1,  # Default: 1 (Integer)

    # Advanced Defaults (Integers for clean display)
    "FFT_CUTOFF_EXPONENT": 6,  # n=6 (2^6 = 64)
    "BUTTERWORTH_ORDER": 2,  # Order=2
    "PADDING_DIVISOR_EXPONENT": 2,  # Relative Order=2 (Ratio = 1/2^2 = 0.25x width)
...
class TomoPipeline:
    ...
    def run(self, input_dir: str) -> bool:
        ...
        print("--- Starting 3D Reconstruction and Post-Processing Task (v25) ---\n")
    ...
    def _apply_fft_filter(self) -> bool:
        """
        Applies FFT Filter (Advanced):
        1. Single-Side Mirror Padding (Left):
           - Formula: floor( Original Width / 2^(relative_order) ).
           - Example: relative_order=1 -> Width/2. relative_order=2 -> Width/4.
           - Limited to max(Original Width).
        2. Filter: Vertical Notch + Horizontal Butterworth Protection (User Order).
        3. Cropping: Remove left padding.
        """
        tolerance = self.config["TOLERANCE_VALUE"]
        cutoff_exponent = self.config.get("FFT_CUTOFF_EXPONENT", 6.0)
        padding_divisor_exponent = self.config.get("PADDING_DIVISOR_EXPONENT", 2.0)  # Relative order
        butterworth_order = self.config.get("BUTTERWORTH_ORDER", 2)

        power_of_2_cutoff = 2.0 ** cutoff_exponent

        # Calculate padding ratio based on relative order: 1 / 2^order
        # e.g. order 1 -> 1/2 = 0.5
        ...
        if CUPY_AVAILABLE:
            try:
                ...
                rows, cols_orig = gpu_volume[0].shape

                # --- Advanced Padding Logic (Relative) ---
                # Formula: Original Width * (1 / 2^relative_order)
                pad_width = int(cols_orig * padding_ratio)
                if pad_width < 1: pad_width = 1
                # [FIX] Limit padding to original width to prevent dimension mismatch
                ...
                # Cutoff threshold logic based on padded width
                # Maintaining ratio: Width / (2^n)
                cutoff_threshold = cols_padded / power_of_2_cutoff
                if cutoff_threshold < 1.0: cutoff_threshold = 1.0

                print(
                    f"FFT Config -> T:{tolerance}, Sigma:{sigma:.2f}, n:{cutoff_exponent}, RelOrder:{padding_divisor_exponent}, Order:{butterworth_order}")
                print(f"  -> Pad Width: {pad_width}px (Left), Cutoff: {cutoff_threshold:.2f}px")
                ...
                # 2. Horizontal Butterworth Protection Mask X
                epsilon = 1e-6
                ...
                for i, polar_slice_gpu in enumerate(gpu_volume):
                    # --- Step A: Single-Side Mirror Padding (Left) ---
                    ...
                    # --- Step C: Crop back (remove left padding) ---
                    filtered_gpu_volume[i] = result_real[:, pad_width:]
        ...
        if not CUPY_AVAILABLE or use_cpu_fallback:
            ...
            rows, cols_orig = sample_slice.shape

            # CPU Padding Logic (Relative)
            pad_width = int(cols_orig * padding_ratio)
            if pad_width < 1: pad_width = 1
            if pad_width > cols_orig: pad_width = cols_orig
            ...
            print(
                f"FFT Config (CPU) -> T:{tolerance}, Sigma:{sigma:.2f}, n:{cutoff_exponent}, RelOrder:{padding_divisor_exponent}, Pad:{pad_width}, Cutoff:{cutoff_threshold:.2f}")
    ...
    def _save_volume(self, volume_type: str):
        ...
        # Modified Filename Logic: Remove "RECON_"
        if volume_type == "RECON":
            output_filename = f"{self.base_name}_{param_str}.tif"
        else:
            # Keep "POLAR_" etc for intermediate steps if saved
...
class App(tk.Tk):
    def __init__(self, defaults: Dict[str, Any]):
        ...
        self.title("3D Tomography Reconstruction Tool v25")
    ...
    def _create_widgets(self):
        ...
        main_frame.rowconfigure(3, weight=1)  # Log frame is row 3 in new layout
        ...
        # 1. Standard Filter Parameters (Left side of top container)
        ...
        for i, (key, label) in enumerate(param_defs.items()):
            ...
            # Formatting logic: if float is like 1.0, show as 1
        ...
        # 2. Advanced Parameters (Right side of top container)
        ...
        for i, (key, label) in enumerate(adv_defs.items()):
            ...
            # Formatting logic for advanced params too
            val = self.defaults.get(key, "")
            if isinstance(val, float) and val.is_integer():
                val_str = str(int(val))
            else:
        ...
        # 3. Save Selection (Row 1 of main_frame)
    ...
    def _on_processing_complete(self, success: bool):
        if success:
            self.bell()
            # Transient popup that closes automatically
