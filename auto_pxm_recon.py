...
def get_image_dimensions(file_size):
...
def parse_pxm_to_array(pxm_path):
    file_size = os.path.getsize(pxm_path)
    width, height = get_image_dimensions(file_size)
    if not width:
        raise ValueError(f"Invalid file size for {os.path.basename(pxm_path)}")
    ...
    return np.array(pixel_data, dtype=np.uint16).reshape((height, width))
...
class TomoPipeline:
    def __init__(self, config: Dict[str, Any], sample_dir: str, log_func):
        ...
        self.pxm_dir = os.path.join(sample_dir, PXM_FOLDER_NAME)
    ...
    def run(self) -> bool:
        ...
        if self.config.get("SAVE_PROJ_TIF"):
            os.makedirs(self.tif_dir, exist_ok=True)
    ...
    def _prepare_projections(self) -> bool:
        self.log("Loading background PXM files...")
        bg0_path = os.path.join(self.pxm_dir, "PXMs_00_0000_0000.pxm")
        bg1_path = os.path.join(self.pxm_dir, "PXMs_00_0000_0001.pxm")
        
        if not os.path.exists(bg0_path) or not os.path.exists(bg1_path):
            self.log("Background PXM files missing. Aborting.")
            return False

        try:
            bg0 = parse_pxm_to_array(bg0_path).astype(np.float32)
            bg0 = np.flipud(bg0)
            bg0[bg0 == 0] = 1.0
            bg1 = parse_pxm_to_array(bg1_path).astype(np.float32)
            bg1 = np.flipud(bg1)
            bg1[bg1 == 0] = 1.0
        except Exception as e:
            self.log(f"Error loading backgrounds: {e}")
            return False

        proj0_path = os.path.join(self.pxm_dir, "PXMs_04_0000_0000.pxm")
        proj180_path = os.path.join(self.pxm_dir, "PXMs_04_0000_0720.pxm")
        
        if not os.path.exists(proj0_path) or not os.path.exists(proj180_path):
            self.log("0 or 180 degree PXM files missing. Aborting.")
            return False

        self.log("Calculating center of rotation...")
        proj0 = parse_pxm_to_array(proj0_path).astype(np.float32)
        proj0 = np.flipud(proj0)
        proj180 = parse_pxm_to_array(proj180_path).astype(np.float32)
        proj180 = np.flipud(proj180)
        ...
        original_height, original_width = img0.shape
        test_crop = crop_image(img0, center_idx, original_width)
        ...
        self.log(f"Processing 721 projections (Correct -> Crop -> Bin)...")
        update_interval = max(1, 721 // 10)
        
        for i in range(721):
            pxm_name = f"PXMs_04_0000_{i:04d}.pxm"
            pxm_path = os.path.join(self.pxm_dir, pxm_name)
            
            if not os.path.exists(pxm_path):
                self.log(f"Missing projection {pxm_name}. Filling with zeros.")
                continue
                
            arr = parse_pxm_to_array(pxm_path).astype(np.float32)
            arr = np.flipud(arr)
            ...
            if self.config.get("SAVE_PROJ_TIF"):
                tif_path = os.path.join(self.tif_dir, f"PXMs_04_0000_{i:04d}.tif")
                tifffile.imwrite(tif_path, corrected)
            ...
            # 根據 Beer-Lambert 定律應用負自然對數: -log(I/I_0)
            # 此步驟移至裁切與縮減取樣之後，以確保儲存的TIF為穿透影像，且重建前數據為吸收數據。
...
class App(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        save_defs = {
            ...
            "SAVE_PROJ_TIF": "5. Save TIF Projection Files (to TIF folder)"
        }
        ...
        try:
            while not self.is_monitoring.is_set():
                ...
                for item in os.listdir(folder_path):
                    ...
                    pxm_files = [f for f in os.listdir(pxm_dir) if f.lower().endswith(".pxm")]
                    if len(pxm_files) == 723:
                        has_tif = False
                        if os.path.isdir(auto_raw_dir):
                            tif_files = [f for f in os.listdir(auto_raw_dir) if f.lower().endswith(('.tif', '.tiff'))]
                            if len(tif_files) > 0:
                                has_tif = True
