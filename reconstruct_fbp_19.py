...
Workflow Overview
================================================================================
This script provides a Graphical User Interface (GUI) for a complete 3D
tomography reconstruction and image post-processing pipeline, starting from
raw projection images to the final processed 3D volume.

The main processing steps are as follows:

1.  **GUI Interface**:
    -   A single main window allows the user to configure all parameters,
        select intermediate/final results to save, and choose the folder
        containing the projection images.

2.  **Data Loading and Pre-processing (CPU)**:
    -   After clicking "Start," the application reads all projection images
        (TIF format) from the specified directory.
    -   Images are downscaled based on the user-defined "Binning Factor" to
        reduce computational load and speed up processing.

3.  **GPU Reconstruction (ASTRA Toolbox)**:
    -   Utilizes the `FBP_CUDA` algorithm from the ASTRA Toolbox for efficient,
        slice-by-slice 3D volume reconstruction using Filtered Back-Projection
        on the GPU.

4.  **Ring Artifact Correction (Polar Transform + FFT Filtering)**:
    a.  **Cartesian to Polar Transform (CPU - OpenCV)**:
        -   The reconstructed 3D volume is converted slice-by-slice from
            Cartesian to Polar coordinates. This transform converts concentric
            "ring artifacts" into vertical stripes.
    b.  **FFT Band-stop Filtering (GPU/CPU)**:
        -   A Fast Fourier Transform (FFT) is applied to the polar-coordinate images.
        -   A Gaussian band-stop filter is used in the frequency domain to
            suppress the frequencies corresponding to the vertical stripes.
        -   This step is GPU-accelerated if CuPy is available; otherwise, it
            falls back to SciPy on the CPU.
    c.  **Polar to Cartesian Transform (CPU - OpenCV)**:
        -   The filtered polar images are transformed back to Cartesian coordinates.

5.  **3D Volume Filtering (Denoising and Smoothing)**:
    a.  **3D Median Filter (GPU/CPU)**:
        -   A median filter is applied to the entire 3D volume to remove
            salt-and-pepper noise.
        -   This step is also GPU-accelerated with CuPy if available.
    b.  **3D Gaussian Filter (CPU - SciPy)**:
        -   A Gaussian filter is applied to smooth the volume and reduce
            high-frequency noise.
        -   This step is always performed on the CPU using SciPy to ensure
            consistent results.

6.  **Smart Execution and Saving**:
    -   The application intelligently determines the final required processing
        step based on the user's output selections.
    -   The pipeline runs only up to the last necessary step, skipping all
        subsequent computations to save time.
    -   Output files are saved with a standardized naming convention that
        includes all relevant processing parameters.

================================================================================
Required Libraries
================================================================================
- tkinter:          Python's built-in GUI library.
- os, glob, sys:    For file and system path manipulation.
- threading:        To run the processing task in a background thread, keeping the GUI responsive.
- typing:           For type hints to improve code readability.
- numpy:            Core library for numerical and array operations.
- tifffile:         For reading and writing TIF image stacks.
- astra-toolbox:    High-performance toolbox for tomographic reconstruction (GPU accelerated).
- opencv-python:    Used for Cartesian/Polar coordinate transformations (cv2).
- scikit-image:     Used for image downscaling (binning).
- cupy (cupyx):     (Optional, highly recommended) For GPU-accelerated FFT and 3D median filtering.
- scipy:            Used for CPU-based filtering and as a fallback for FFT.
"""
...
class TomoPipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.processing_steps = [
            ...
            {"name": "fft_filter", "method": self._apply_fft_filter, "save_key": "SAVE_POLAR_FILTERED", "type": "POLAR"},
    ...
    def run(self, input_dir: str) -> bool:
        ...
        if not last_step_to_run:
            print("Warning: No output file was selected. The program will not produce any output.")
            return False
    ...
    def _save_volume(self, volume_type: str):
        ...
        if "median" in self.file_params and self.file_params['median'] > 0: param_parts.append(f"M{self.file_params['median']}")
        if "gauss" in self.file_params and self.file_params['gauss'] > 0: param_parts.append(f"G{self.file_params['gauss']:.1f}".replace('.0', ''))
...
class App(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        # --- Output Selection ---
        save_frame = LabelFrame(main_frame, text="Select Files to Output (Pipeline runs to the last checked item)", padx=10, pady=10)
        ...
        save_defs = {
            ...
            "SAVE_POLAR_FILTERED": "3. FFT-Filtered Polar Volume (POLAR)", "SAVE_RECON_FILTERED": "4. FFT-Filtered Reconstructed Volume (RECON)",
        ...
        folder_label = tk.Label(folder_select_frame, textvariable=self.folder_path, fg="blue", wraplength=300, justify="left")
...
def main():
    ...
    try:
        ...
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror("Critical Error", f"An unexpected error occurred and the application will close.\n\nDetails:\n{e}")
