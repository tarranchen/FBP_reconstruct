...
7.  **3D濾波 (部分GPU加速)**：
    -   **中值濾波**：對體積進行3D中值濾波，以去除椒鹽雜訊(Salt-and-pepper noise)。若偵測到 CuPy，此步驟會在 GPU 上執行。
    -   **高斯模糊 (CPU)**：對體積進行3D高斯模糊，**此步驟固定使用 Scipy 在 CPU 上執行**，以平滑影像並抑制高頻雜訊。
...
- cupy (cupyx): (可選，推薦) 用於GPU加速的3D中值濾波與FFT。
- scipy: 用於CPU上的3D濾波與FFT。
"""
...
try:
    ...
    CUPY_AVAILABLE = True
    print("成功載入 CuPy，將使用 GPU 進行中值濾波與FFT加速。")
except ImportError:
    CUPY_AVAILABLE = False
    print("警告：未找到 CuPy。FFT與中值濾波將在 CPU 上執行，速度可能較慢。")

# --- 無論 CuPy 是否存在，都導入 Scipy 的濾波器 ---
# 高斯模糊將固定使用 Scipy
...
class TomoPipeline:
    ...
    def _apply_gaussian_filter(self) -> bool:
        ...
        # 直接調用 scipy.ndimage 中的 gaussian_filter
...
def main():
    ...
    print("3D斷層掃描重建工具已啟動。")
    print("高斯模糊將固定在 CPU 上執行。")
    if not CUPY_AVAILABLE:
        print("提示：若要啟用中值濾波與FFT的GPU加速，請安裝 CuPy 函式庫。")
    app.mainloop()
