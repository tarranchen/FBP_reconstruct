...
7. (可選) 對體積進行3D中值濾波，以去除椒鹽雜訊。
8. (可選) 對體積進行3D高斯模糊，以平滑影像。
9. 根據設定，選擇性地儲存每一步驟的結果檔案，並自動跳過不必要的運算。
...
CONFIG = {
    ...
    # 3D 高斯模糊標準差 Sigma (pixel)
    "GAUSSIAN_SIGMA": 3,

    # --- 各步驟存檔選項 ---
    # 設定為 True 表示要儲存該步驟的檔案，False 則不儲存。
    # 程式會自動執行到最後一個設定為 True 的步驟，並跳過後續運算。
    ...
    "SAVE_RECON_FILTERED": False,  # 是否儲存濾波後轉回直角座標的檔案 (recon_filtered)
    "SAVE_RECON_FILTERED_MEDIAN": True,  # 是否儲存中值濾波後的檔案
    "SAVE_RECON_FILTERED_MEDIAN_GAUSS": True,  # 是否儲存高斯模糊後的檔案
}
...
def apply_3d_median_filter(volume: np.ndarray, size: int) -> np.ndarray:
    """對3D體積應用中值濾波器。"""
    if size <= 1:
        return volume
    print(f"開始3D中值濾波 (核心大小: {size}x{size}x{size})...")
    filtered_volume = median_filter(volume, size=size)
    print("   3D中值濾波完成。")
    return filtered_volume


def apply_3d_gaussian_filter(volume: np.ndarray, sigma: float) -> np.ndarray:
    """對3D體積應用高斯模糊。"""
    if sigma <= 0:
        return volume
...
def save_volume_as_tif(
    ...
    Args:
        ...
        suffix (str): 檔名後綴，用以區分不同處理結果。
        cfg (dict): 全域組態字典。
        params (dict): 包含該檔案所有相關濾波器參數的字典。
    """
    z, y, x = volume_data.shape

    param_str = ""
    if "tolerance" in params:
        param_str += f"_T{params['tolerance']}"
    if "median" in params:
        param_str += f"_M{params['median']}"
    if "gauss" in params:
        param_str += f"_G{params['gauss']}"

    output_filename = f"{base_name}_{suffix}{param_str}_B{cfg['BINNING_FACTOR']}_{x}x{y}x{z}.tif"
...
def main():
    ...
    # 檢查是否設定了任何存檔選項
    if not any(v for k, v in CONFIG.items() if k.startswith("SAVE_")):
        print("警告：在組態中沒有設定任何存檔選項 (SAVE_...)，程式不會產生任何輸出檔案。")
        # return # 可選擇在此直接終止
    ...
    projections, angles, _, _ = load_projections(
        input_dir, CONFIG["FILE_PATTERN"], CONFIG["BINNING_FACTOR"]
    )
    if projections is None: return
    ...
    if CONFIG["SAVE_RECON_RAW"]:
        save_volume_as_tif(reconstructed_volume, folder_name, "recon", CONFIG, {})

    # 檢查是否需要執行後續步驟 (從後往前檢查)
    if not (CONFIG["SAVE_POLAR_RAW"] or CONFIG["SAVE_POLAR_FILTERED"] or CONFIG["SAVE_RECON_FILTERED"] or CONFIG[
        "SAVE_RECON_FILTERED_MEDIAN"] or CONFIG["SAVE_RECON_FILTERED_MEDIAN_GAUSS"]):
        print("\n後續步驟皆未設定存檔，處理流程提前結束。")
        return
    ...
    if CONFIG["SAVE_POLAR_RAW"]:
        save_volume_as_tif(polar_volume, folder_name, "polar", CONFIG, {})

    if not (CONFIG["SAVE_POLAR_FILTERED"] or CONFIG["SAVE_RECON_FILTERED"] or CONFIG["SAVE_RECON_FILTERED_MEDIAN"] or
            CONFIG["SAVE_RECON_FILTERED_MEDIAN_GAUSS"]):
        print("\n後續步驟皆未設定存檔，處理流程提前結束。")
        return

    raw_tolerance = CONFIG["TOLERANCE_VALUE"]
    processed_tolerance = 0.001 * (raw_tolerance ** 2.5)
    filtered_polar_volume = fft_filter_vertical_stripes(polar_volume, processed_tolerance)

    current_params = {"tolerance": raw_tolerance}
    if CONFIG["SAVE_POLAR_FILTERED"]:
        save_volume_as_tif(filtered_polar_volume, folder_name, "polar_filtered", CONFIG, current_params)

    # --- Inverse Polar & Cartesian Filters ---
    if not (CONFIG["SAVE_RECON_FILTERED"] or CONFIG["SAVE_RECON_FILTERED_MEDIAN"] or CONFIG[
        "SAVE_RECON_FILTERED_MEDIAN_GAUSS"]):
        print("\n後續步驟皆未設定存檔，處理流程提前結束。")
        return

    _, y_dim, x_dim = reconstructed_volume.shape
    filtered_cartesian_volume = convert_polar_to_cartesian(filtered_polar_volume, (y_dim, x_dim))

    if CONFIG["SAVE_RECON_FILTERED"]:
        save_volume_as_tif(filtered_cartesian_volume, folder_name, "recon_filtered", CONFIG, current_params)

    if not (CONFIG["SAVE_RECON_FILTERED_MEDIAN"] or CONFIG["SAVE_RECON_FILTERED_MEDIAN_GAUSS"]):
        print("\n後續步驟皆未設定存檔，處理流程提前結束。")
        return

    median_filtered_volume = apply_3d_median_filter(filtered_cartesian_volume, CONFIG["MEDIAN_FILTER_SIZE"])
    current_params["median"] = CONFIG["MEDIAN_FILTER_SIZE"]
    if CONFIG["SAVE_RECON_FILTERED_MEDIAN"]:
        save_volume_as_tif(median_filtered_volume, folder_name, "recon_filtered_median", CONFIG, current_params)

    if not CONFIG["SAVE_RECON_FILTERED_MEDIAN_GAUSS"]:
        print("\n後續步驟皆未設定存檔，處理流程提前結束。")
        return

    gaussian_filtered_volume = apply_3d_gaussian_filter(median_filtered_volume, CONFIG["GAUSSIAN_SIGMA"])
    current_params["gauss"] = CONFIG["GAUSSIAN_SIGMA"]
    if CONFIG["SAVE_RECON_FILTERED_MEDIAN_GAUSS"]:
        save_volume_as_tif(gaussian_filtered_volume, folder_name, "recon_filtered_median_gauss", CONFIG, current_params)
