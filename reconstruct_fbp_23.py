# -*- coding: utf-8 -*-
"""
3D Tomography Reconstruction and Post-Processing Tool v23 (Tkinter GUI)
(English Version: Mirror Padding + Overlay Logic + Butterworth Filter)

================================================================================
Change Log (v23):
1. [Language] All comments and GUI text converted to English.
2. [GUI] Removed 'Ring Removal Cutoff' input field from GUI.
   - Internal default set to exponent n=6 (2^6 = 64).
3. [Config] Updated default parameters:
   - Tolerance: 25
   - Median Radius: 1
   - Gaussian Sigma: 1
4. [Algorithm] Retained advanced FFT logic:
   - Mirror Padding (prevents boundary artifacts).
   - Overlay/Maximum masking (robust protection).
...
DEFAULT_CONFIG = {
    "FILE_PATTERN": '*.tif',
    "BINNING_FACTOR": 1,
    "TOLERANCE_VALUE": 25,  # Updated default: 25
    "MEDIAN_FILTER_RADIUS": 1,  # Updated default: 1
    "GAUSSIAN_SIGMA": 1,  # Updated default: 1
    # "FFT_CUTOFF_EXPONENT": 6.0 # Hardcoded internally as requested
...
class TomoPipeline:
    ...
    def run(self, input_dir: str) -> bool:
        ...
        # Dynamic Step Logic: Skip Polar/FFT if Tolerance <= 0
    ...
    def _apply_fft_filter(self) -> bool:
        """
        Applies FFT Filter:
        1. Mirror Padding: Flip and concatenate to double width (removes boundary artifacts).
        2. Filter: Vertical Notch + Horizontal Butterworth Protection (Overlay/Maximum).
        3. Cropping: Restore original width.
        """
        tolerance = self.config["TOLERANCE_VALUE"]
        # Use hardcoded exponent n=6 as requested
        cutoff_exponent = self.config.get("FFT_CUTOFF_EXPONENT", 6.0)
        # Calculate actual divisor 2^n
        cutoff_divisor = 2.0 ** cutoff_exponent
        ...
        if CUPY_AVAILABLE:
            try:
                ...
                # --- Filter Parameter Calculation (Based on Padded Width) ---
                ...
                print(
                    f"FFT Filter Config -> T: {tolerance}, Sigma: {sigma:.2f}, Fixed n: {cutoff_exponent}, Divisor: {cutoff_divisor:.1f}, Cutoff: {cutoff_threshold:.2f} px")

                # Coordinate System (Using Padded Width)
                y_coords = cp.arange(rows) - (rows // 2)
                x_coords = cp.arange(cols_padded) - (cols_padded // 2)

                # 1. Vertical Notch Y
                # Function: Suppress vertical lines (0 at center, 1 at edges)
                notch_y = 1.0 - cp.exp(-(y_coords ** 2) / (2 * sigma ** 2))
                notch_y = notch_y.reshape(-1, 1)

                # 2. Horizontal Butterworth Protection Mask X
                # Function: Protect center low-freq/thick objects (1 at center, 0 at edges)
                epsilon = 1e-6
                order = 2
                ...
                for i, polar_slice_gpu in enumerate(gpu_volume):
                    ...
                    # --- Step C: Crop back to original ---
                    # Keep right half (Original)
                    filtered_gpu_volume[i] = result_real[:, cols_orig:]
        ...
        if not CUPY_AVAILABLE or use_cpu_fallback:
            ...
            print(
                f"FFT Filter Config (CPU) -> T: {tolerance}, Sigma: {sigma:.2f}, Fixed n: {cutoff_exponent}, Divisor: {cutoff_divisor:.1f}, Cutoff: {cutoff_threshold:.2f} px")
            ...
            # 2. Horizontal Butterworth Protection
            epsilon = 1e-6
            order = 2
            ...
            for i, polar_slice in enumerate(self.data_volume):
                ...
                # Crop
                filtered_slices.append(result_real[:, cols_orig:])
...
class App(tk.Tk):
    def __init__(self, defaults: Dict[str, Any]):
        ...
        self.title("3D Tomography Reconstruction Tool")  # Version update
    ...
    def _create_widgets(self):
        ...
        # NOTE: 'FFT_CUTOFF_EXPONENT' is removed from GUI input as requested.
        # It uses the default value (6.0) internally.
        param_defs = {
    ...
    def _start_processing_thread(self):
        ...
        try:
            user_params = {
                ...
                # Inject the hardcoded cutoff exponent here
                "FFT_CUTOFF_EXPONENT": 6.0
            }
