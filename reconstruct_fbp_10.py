...
4. 將重建後的笛卡爾座標（Cartesian）3D體積儲存為TIF檔案。
5. 將笛卡爾座標體積轉換為極座標（Polar coordinates），以方便某些特定分析（例如分析管狀結構）。
6. 將轉換後的極座標3D體積儲存為另一個TIF檔案。
7. (可選) 對極座標影像進行FFT濾波，以去除因探測器不一致造成的環狀偽影(Ring Artifacts)。
8. 將濾波後的極座標3D體積儲存為最終的TIF檔案。
...
def convert_volume_to_polar(cartesian_volume: np.ndarray) -> np.ndarray:
    ...
    height, _, width = cartesian_volume.shape
    center_x = width / 2.0
    center_y = height / 2.0
    ...
    oversampled_height = int(width * np.pi)
...
def fft_filter_vertical_stripes(
    ...
    """
    print(f"開始FFT濾波以去除垂直條紋 (容忍度: {tolerance_percent}%) ...")
    ...
    for polar_slice in tqdm(polar_volume, desc="FFT濾波進度"):
        ...
        # 因此，我們建立一個水平帶狀的遮罩來抑制這些頻率。
        band_width = max(1, int((rows * tolerance_percent / 100) / 2))

        # 遮罩中心水平帶的值設為0，其餘為1
        fft_slice[center_row - band_width: center_row + band_width + 1, :] = 0
...
def save_volume_as_tif(
    ...
    # 組合輸出檔名，格式為：基礎檔名_後綴_B(合併因子)_(維度X)x(維度Y)x(維度Z).tif
    output_filename = f"{base_name}_{suffix}_B{binning_factor}_{x}x{y}x{z}.tif"
...
def main():
    ...
    # 步驟 2: 讀取與預處理影像
    projections, angles, _, _ = load_projections(input_dir, FILE_PATTERN, BINNING_FACTOR)
    ...
    filtered_polar_volume = fft_filter_vertical_stripes(polar_volume, tolerance_percent=0.1)
