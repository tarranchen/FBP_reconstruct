...
(Fixed Version: Overlay Logic + Butterworth Filter + Exponential Cutoff Input)

================================================================================
修正說明 (Fix Log):
1. [GUI] "Ring Removal Cutoff" 選項現在接受指數 n (2^n)。
   - 介面文字更新為 "Ring Removal Cutoff (1/2^n):"。
   - 預設值改為 6.0 (即 2^6 = 64)。
2. [Algorithm] FFT 濾波器邏輯更新：
   - 使用遮罩疊加法 (Overlay/Maximum)。
   - Cutoff Divisor 計算方式改為 2 ** input_value。
3. [Logic] 維持 Tolerance <= 0 跳過步驟的邏輯。
================================================================================
"""
...
DEFAULT_CONFIG = {
    ...
    "BINNING_FACTOR": 1,
    "TOLERANCE_VALUE": 40,
    "FFT_CUTOFF_DIVISOR": 6.0,  # 控制水平保護區寬度 (1/2^n)，預設 2^6 = 64
...
class TomoPipeline:
    ...
    def _apply_fft_filter(self) -> bool:
        """
        應用 FFT 濾波：垂直高斯陷波 + 水平巴特沃斯保護 (Overlay/Maximum)
        - Notch Y: 消除垂直偽影。
        - Protect X: 保護低頻寬物體 (使用 Butterworth 平滑過渡)。
        """
        tolerance = self.config["TOLERANCE_VALUE"]
        # 從 GUI 獲取參數，此處為指數 n
        cutoff_exponent = self.config["FFT_CUTOFF_DIVISOR"]
        # 計算實際的除數 2^n
        ...
        if CUPY_AVAILABLE:
            try:
                ...
                print(
                    f"FFT Filter Config -> T: {tolerance}, Sigma: {sigma:.2f}, Input n: {cutoff_exponent}, Divisor(2^n): {cutoff_divisor:.1f}, Cutoff: {cutoff_threshold:.2f} px")
                ...
                # 2. 水平巴特沃斯保護遮罩 (Protect X)
                # 作用：保護中心低頻/寬物體 (中心為1，邊緣為0)
                # 使用 Low-pass Butterworth 公式
                epsilon = 1e-6
                order = 2  # Order=2 提供平滑過渡
                protect_x = 1.0 / (1.0 + (cp.abs(x_coords) / (cutoff_threshold + epsilon)) ** (2 * order))
                protect_x = protect_x.reshape(1, -1)

                # 3. 疊加邏輯 (Overlay/Maximum)
                # 取兩者最大值。只要 Protection 為 1 (中心)，結果就是 1 (保留)。
        ...
        if not CUPY_AVAILABLE or use_cpu_fallback:
            ...
            print(
                f"FFT Filter Config (CPU) -> T: {tolerance}, Sigma: {sigma:.2f}, Input n: {cutoff_exponent}, Divisor(2^n): {cutoff_divisor:.1f}, Cutoff: {cutoff_threshold:.2f} px")
...
class App(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        param_defs = {
            ...
            "FFT_CUTOFF_DIVISOR": "Ring Removal Cutoff (1/2^n):",
    ...
    def _start_processing_thread(self):
        ...
        try:
            user_params = {
                ...
                "FFT_CUTOFF_DIVISOR": float(self.entries["FFT_CUTOFF_DIVISOR"].get()),
